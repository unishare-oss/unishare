...
export function ArcadePostCard({ post, index = 0, trending = false }: ArcadePostCardProps) {
  ...
  return (
    <div className="relative" style={{ '--stagger': index } as CSSProperties}>
      <Link href={`/posts/${post.id}`} className="block" onClick={() => markRead(post.id)}>
        <article className="arcade-card grid grid-cols-[56px_1fr] sm:grid-cols-[80px_1fr] bg-card rounded-2xl overflow-hidden cursor-pointer">
          {/* Type spine */}
          <div
            className={cn(
              ...
              <div className="flex flex-wrap gap-1.5 mb-3">
                {post.tags.map((tag) => (
                  <button
                    ...
                    className="font-mono text-[10px] font-bold px-2 py-0.5 rounded-lg bg-background text-text-muted border-2 border-border hover:border-amber hover:text-amber transition-colors"
                  >
                    {tag.name}
                  </button>
                ))}
