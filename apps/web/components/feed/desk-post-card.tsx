  ...
  return (
    ...
    >
      <Link href={`/posts/${post.id}`} className="block h-full" onClick={() => markRead(post.id)}>
        <article
          className={cn(
            'desk-paper relative h-full bg-card border border-border-strong/50 px-5 pt-7 pb-4 cursor-pointer',
        ...
        >
          ...
          {trending && (
            <span
              aria-hidden
              className="desk-pin absolute -top-1.5 right-6 size-3.5 rounded-full"
            />
          )}
            ...
            <div className="flex flex-wrap gap-1.5 mb-2.5">
              {post.tags.map((tag) => (
                <button
                  ...
                  className="font-mono text-[10px] px-2 py-0.5 rounded-[4px] bg-muted/70 text-text-muted border border-border hover:border-amber hover:text-amber transition-colors"
                >
                  {tag.name}
                </button>
              ))}
          ...
          <span
            aria-hidden
            className="absolute right-0 bottom-0 size-8 bg-[linear-gradient(135deg,transparent_49%,color-mix(in_srgb,var(--foreground)_12%,transparent)_50%)]"
          />
        </article>
      </Link>
