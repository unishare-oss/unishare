...
| `components/chat/group-chat-dialog.tsx`       | Renamed from `create-group-dialog.tsx`; extended with invite mode; group name field now uses **Zod + React Hook Form** (shadcn Form primitives) with `min(1)` / `max(30)` validation, live character counter, and `FormMessage` error display; resolver is conditionally applied (create mode only — invite mode bypasses name validation); all inner buttons carry explicit `type="button"` to prevent accidental form submission |
| `components/chat/group-chat-user-row.tsx`     | **New** — co-located sub-components `UserRow` (selectable user list row with avatar + checkmark indicator) and `SelectedBadge` (removable chip); extracted from `group-chat-dialog.tsx` to keep the dialog lean                                                                                                                                                                                                                    |
...
- **Zod + RHF on group name** — name field is now validated client-side (`min(1)`, `max(30)`) with a live character counter; `zodResolver` is only attached in create mode so invite mode submits without triggering name validation.
- **`UserRow` / `SelectedBadge` extraction** — the user list row and selected chip were inlined in the dialog; extracted to `group-chat-user-row.tsx` to keep the dialog file focused on form logic. Both components are co-located with the dialog in `components/chat/`.
- **Single `<Form><form>` wrapper** — rather than branching the entire JSX on `mode`, a single form element wraps both modes; the resolver is conditionally `undefined` for invite mode, which lets RHF's `handleSubmit` call through without schema validation.
